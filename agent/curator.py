...
# Writing standard — distilled from writing-for-agents (mandatory for any
# skill you patch/create via skill_manage). Injected into curator prompt
# because the curator fork runs with skip_context_files=True and never
# reads .hermes.md/SOUL.md. Covers: pointer front-load, hierarchy,
# leading words, pruning, no negation, steps with Done when.
CURATOR_WRITING_STANDARD = (
    "\n\n## Writing standard (mandatory for any skill you patch/create)\n"
    "Every skill you patch/create MUST also satisfy `writing-for-agents` + "
    "`SKILL-MECHANICS.md`:\n"
    "- **Pointer:** front-load leading word in `description` (first 57 chars "
    "hold trigger class), one trigger per branch, cut identity body already carries.\n"
    "- **Hierarchy:** steps (primary, each ends with `Done when` checável+exaustivo) "
    "→ flat reference (consulted on demand) → disclosed reference (behind pointer, "
    "load only when branch fires). Branch test: inline what every branch needs; "
    "disclose what only some branches need.\n"
    "- **Leading words:** use single token anchors (`graph`, `diamond`, `fake-edge`, "
    "`stop-rule`, `human-gate`, `kg`, `task-graph`) — recruit priors, collapse restatements.\n"
    "- **Pruning:** single source of truth, no cache of env (ls/config already answers), "
    "no sprawl, no duplication, relevance check each line.\n"
    "- **No negation:** phrase positive target behaviour; `Do NOT` drags forbidden "
    "concept into context.\n"
    "Apply before any `skill_manage` patch/create/write_file. A skill that fails "
    "this is not `keep` — fix it.\n"
)

# Append writing standard to the review prompt (fork never reads .hermes.md,
# so this is the only path that reaches the curator).
CURATOR_REVIEW_PROMPT = CURATOR_REVIEW_PROMPT + CURATOR_WRITING_STANDARD
