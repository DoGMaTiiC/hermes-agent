                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # Legacy rows predate provider tagging. Do not send
                            # unknown-origin encrypted blobs to xAI, because a
                            # blob minted by Codex/OpenAI cannot be decrypted by
                            # Grok after a provider switch within one session.
                            if is_xai_responses and not item_provider_norm:
                                continue
                            if (
                                current_reasoning_provider
